...
class VideoPlayerSurface extends ConsumerStatefulWidget {
  ...
  /// 应用内快捷键「上一集」（PgUp）要用；全局热键那条走 [PlaybackHotkeyTarget]。
  ...
  /// 不画播放器自带的顶部条（返回 / 标题 / 更多）。独立播放窗口用它 —— 那边窗口
  /// 已经有一条浮在画面上的标题条，两条叠在一起会出现两个返回键和两行标题。
...
class _VideoPlayerSurfaceState extends ConsumerState<VideoPlayerSurface> {
  /// 应用内快捷键的焦点节点：播放页 / 全屏播放器持有焦点时键位生效，
  /// 点进评论 / 搜索等文本框后事件不再路过这里，天然不误触。
  final FocusNode _focusNode = FocusNode(debugLabel: 'video-player-surface');
  ...
  void _restartTimer() {
    ...
    final seconds =
        ref.read(settingsProvider).valueOrNull?.playerControlsTimeoutSeconds ??
        4;
  ...
  void _onPointerSignal(PointerSignalEvent event) {
    ...
    GestureBinding.instance.pointerSignalResolver.register(event, (resolved) {
      ...
      // 上滚加、下滚减
      final step =
          (-scroll.scrollDelta.dy / _scrollUnitsPerNotch * _volumeScrollStep)
  ...
  void _showVolumeHud(double volume) {
    ...
    _volumeHudTimer = Timer(const Duration(milliseconds: 900), () {
      if (mounted) setState(() => _volumeHud = null);
    });
  }

  void _toggleControls() {
    if (_locked) return;
    // 点画面同时也是把焦点拿回播放器（否则方向键会去滚列表而不是快进快退）。
    if (!_focusNode.hasFocus) _focusNode.requestFocus();
    setState(() => _showControls = !_showControls);
    _restartTimer();
  }

  // ---- 应用内快捷键（键位表见设置 → 快捷键，编解码在 PlayerHotkeyRegistry） ----
  //
  // 只在焦点落在播放器上时生效；与全局热键（GlobalHotkeys，窗口未激活也生效）
  // 是两套独立分发。Esc 退出全屏是所有播放器的铁律，不查键位表直接写死。

  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    // 长按自动重复（KeyRepeatEvent）只对快进快退 / 音量有意义；播放暂停等
    // 单发动作忽略重复，避免按住空格疯狂开关。
    final repeat = event is KeyRepeatEvent;
    if (event is! KeyDownEvent && !repeat) return KeyEventResult.ignored;
    if (_locked) return KeyEventResult.ignored;
    if (event.physicalKey == PhysicalKeyboardKey.escape) {
      if (!widget.fullscreen) return KeyEventResult.ignored;
      if (!repeat) widget.onFullscreen();
    ...
    final combo = PlayerHotkeyRegistry.formatCombo({
    ...
    if (combo ==
      ...
      if (!repeat) _togglePlayback();
      return KeyEventResult.handled;
    }
    if (combo ==
        PlayerHotkeyRegistry.bindingFor(
          bindings,
          PlayerHotkeyRegistry.seekBackward,
        )) {
      _seekBy(controller, -const Duration(seconds: 5));
      return KeyEventResult.handled;
    }
    if (combo ==
        PlayerHotkeyRegistry.bindingFor(
          bindings,
          PlayerHotkeyRegistry.seekForward,
        )) {
      _seekBy(controller, const Duration(seconds: 5));
      return KeyEventResult.handled;
    }
    if (combo ==
        PlayerHotkeyRegistry.bindingFor(
          bindings,
          PlayerHotkeyRegistry.volumeUp,
        )) {
      _nudgeVolume(controller, _volumeScrollStep);
      return KeyEventResult.handled;
    }
    if (combo ==
        PlayerHotkeyRegistry.bindingFor(
          bindings,
          PlayerHotkeyRegistry.volumeDown,
        )) {
      _nudgeVolume(controller, -_volumeScrollStep);
      return KeyEventResult.handled;
    }
    if (combo ==
        PlayerHotkeyRegistry.bindingFor(bindings, PlayerHotkeyRegistry.mute)) {
      if (!repeat) _toggleMute(controller);
      return KeyEventResult.handled;
    }
    if (combo ==
        PlayerHotkeyRegistry.bindingFor(
          bindings,
          PlayerHotkeyRegistry.fullscreen,
        )) {
      if (!repeat) widget.onFullscreen();
      return KeyEventResult.handled;
    }
    if (combo ==
        PlayerHotkeyRegistry.bindingFor(
          bindings,
          PlayerHotkeyRegistry.previousEpisode,
        )) {
      if (!repeat) widget.onPrevious?.call();
      return KeyEventResult.handled;
    }
    if (combo ==
        PlayerHotkeyRegistry.bindingFor(
          bindings,
          PlayerHotkeyRegistry.nextEpisode,
        )) {
      if (!repeat) widget.onNext?.call();
      return KeyEventResult.handled;
    }
    return KeyEventResult.ignored;
  }
  ...
  /// 键盘调音量与滚轮共用一套 HUD 提示；不写 HUD 的最小音量闪烁。
  void _nudgeVolume(VideoPlayerController controller, double delta) {
    final next = (controller.value.volume + delta).clamp(0.0, 1.0);
    if (next == controller.value.volume) return;
    unawaited(controller.setVolume(next));
    _showVolumeHud(next);
  }
  ...
  void _toggleMute(VideoPlayerController controller) {
    ...
    if (volume > 0) {
      ...
      final restore = _volumeBeforeMute ?? 1.0;
      _volumeBeforeMute = null;
      unawaited(controller.setVolume(restore));
      _showVolumeHud(restore);
    }
  }
  ...
  void _longPress(bool active) {
    ...
    if (active) {
      ...
      final speed = PlaybackSpeedPolicy.longPressSpeed(
        settings,
        isThreeDimensional: PlaybackSpeedPolicy.isThreeDimensional(
          widget.video.genre,
          widget.video.title,
        ),
      );
  ...
  void _dragUpdate(DragUpdateDetails details) {
    ...
    if (duration == Duration.zero) return;
    final sensitivity =
        ref.read(settingsProvider).valueOrNull?.seekSensitivity ?? .35;
    final position =
  ...
  Widget build(BuildContext context) {
    ...
    // 用 Focus 而不是 KeyboardListener：这版 KeyboardListener 的回调不能
    // 返回 handled，方向键会穿透去滚动列表；Focus.onKeyEvent 可以吞掉。
    return Focus(
              ...
              child: GestureDetector(
                ...
                child: Stack(
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      child: IgnorePointer(
                        child: ValueListenableBuilder<bool>(
                          valueListenable: ConfiguredMediaKitVideoPlayer
                              .switchingSuperResolution,
                    ...
                    ValueListenableBuilder<VideoPlayerValue>(
                      valueListenable: controller,
                      builder: (context, value, _) => _showControls && !_locked
                          ? VideoPlayerControls(
                              ...
                              onSuperResolutionSelected:
                                  widget.onSuperResolutionSelected,
                    ...
                    //
                    // 独立播放窗口里不画这一条：那边窗口自己有一条浮在画面上的标题条
                    // （含「回到主界面」与窗口按钮），再叠一条就会变成两个返回键 + 两行标题。
                    if (_showControls && !_locked && !widget.hideTopBar)
                      Positioned(
                        top: 0,
                        left: 0,
                        right: 0,
                        child: _PlayerTopBar(
                          ...
                          onSuperResolutionSelected:
                              widget.onSuperResolutionSelected,
                          onPictureInPicture: () =>
                              _enterPictureInPicture(controller),
                          onKeyframes: widget.fullscreen
                              ? widget.onKeyframes
                              : null,
                          onAddKeyframe: widget.fullscreen
                    ...
                    if (_adjustment != null)
                      Positioned(
                        ...
                        right: 0,
                        child: Center(
                          child: _AdjustmentHud(adjustment: _adjustment!),
                        ),
                      ),
                    // 滚轮调音量的白底提示：固定在画面正中，独立于控制条的显示/隐藏
                    if (_volumeHud != null)
                      Center(child: _VolumeHud(volume: _volumeHud!)),
                    ...
                    ValueListenableBuilder<VideoPlayerValue>(
                      ...
                      builder: (context, value, _) {
                        ...
                        return AnimatedPositioned(
                          ...
                          right: 12,
                          bottom: paused && _showControls && !_locked
                              ? 104
                              : 16,
                          child: IgnorePointer(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            if (fullscreen)
              VideoPlayerFullscreenMoreMenu(
                sources: video.sources,
                quality: quality,
              )
            else
              VideoPlayerPortraitMoreMenu(
                controller: controller,
                video: video,
                quality: quality,
...
class _VideoViewport extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final enhance =
        (settings?.superResolutionMode ?? SuperResolutionMode.off) !=
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final fill =
            aspect == VideoAspectRatio.crop ||
            aspect == VideoAspectRatio.stretch;
        return VideoOutputArea(
          enabled: enhance,
          fill: fill,
          size: Size(
            constraints.maxWidth.isFinite ? constraints.maxWidth * dpr : 0,
            constraints.maxHeight.isFinite ? constraints.maxHeight * dpr : 0,
          ),
          ...
          child: switch (aspect) {
            VideoAspectRatio.auto => Center(
              child: AspectRatio(
                aspectRatio: ratio,
                child: VideoPlayer(controller),
              ),
            ),
            VideoAspectRatio.ratio4x3 => Center(
              child: AspectRatio(
                aspectRatio: 4 / 3,
                child: FittedBox(
                  fit: BoxFit.contain,
            ...
            VideoAspectRatio.crop => FittedBox(
              fit: BoxFit.cover,
            ...
            VideoAspectRatio.stretch => FittedBox(
              fit: BoxFit.fill,
              clipBehavior: Clip.hardEdge,
              child: SizedBox(
                width: ratio * 1000,
...
class _MarqueeTitleState extends State<_MarqueeTitle>
  ...
  void initState() {
    ...
    _controller = AnimationController(
      vsync: this,
      duration: Duration(
        milliseconds: (widget.title.length * 85).clamp(4000, 16000).toInt(),
      ),
    )..repeat();
  }
    ...
    builder: (context, constraints) {
      ...
      return ClipRect(
        child: AnimatedBuilder(
          ...
          child: Text(widget.title, maxLines: 1, style: style),
        ),
      );
    },
  );
...
class _Adjustment {
  ...
  factory _Adjustment.speed(double value) =>
      _Adjustment(_AdjustmentKind.speed, value);
  factory _Adjustment.seek(int delta, Duration position, Duration duration) =>
      _Adjustment(
        _AdjustmentKind.seek,
        0,
        delta: delta,
        position: position,
        duration: duration,
      ...
      ValueListenableBuilder<VideoPlayerValue>(
        ...
        builder: (context, value, _) {
          final next = keyframes
              .where((item) => item >= value.position.inMilliseconds)
              .firstOrNull;
            ...
            child: DecoratedBox(
              ...
              child: Padding(
                ...
                child: Text(
                  AppLocalizations.of(
                    context,
                  )!.keyframeCountdown((remaining / 1000).toStringAsFixed(1)),
